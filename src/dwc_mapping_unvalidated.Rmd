...
taxon %<>% mutate(
  dwc_status = status,
  dwc_references = reference,
  dwc_occurrenceStatus = "present"
)
```
...
```{r}
taxon %<>% select(
  kingdom,scientificName,taxonID,vernacularName,references,language,license,rightsHolder,datasetID,datasetName,occurrenceStatus,nomenclaturalCode,order,phylum,genus,class,status,family
)
```
...
```{r}
distribution %<>% mutate(dwc_threadStatus = recode(rlc_as_published
...
```{r}
distribution %<>% select(
  taxonID,redListCriteria,redListCriteriaEurope,criteria,eventDate,source,locality,occurrenceStatus,countryCode,locationID,threadStatus,establishmentMeans
)
```
