...
dwc_taxon_file = "./data/interim/validated/ValidatedRedLists.csv"
```
...
raw_data <- read_delim("../data/raw/validated/ValidatedRedListsSource.csv",";", escape_double = FALSE, trim_ws = TRUE)
```
...
dwc_taxon_file <- taxon %<>%
  ...
  rename(scientificName = raw_speciesname_as_published) %>%
  rename(taxonID = raw_unique_id) %>%
  
  # add column threadStatus
  mutate(threadStatus = str_replace(raw_rlc_as_published,"Bedreigd","Endangered (EN)" )) %>%
...
rename(vernacularName = raw_speciesname_dutch) %>%  
