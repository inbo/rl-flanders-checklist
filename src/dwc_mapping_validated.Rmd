...
```{r}
taxon %<>% mutate(dwc_taxonomicStatus = str_to_lower(status))
```
...
- `rlc_iucn`: IUCN equivalent of Flemish status. Will be mapped to `threatStatus` (which expects this controlled vocabulary).
...
```{r}
description_ext %<>% mutate(key = recode(key,
  "biotope1" = "biotope",
  "biotope2" = "biotope"
))
```
